...
def wikisearch_engine(index):
    query_engine = index.as_query_engine(response_mode='compact', verbose = True, similarity_top_k=10)
    return query_engine
...
def create_react_agent(MODEL):
    query_engine_tools = [
        QueryEngineTool(
            ...
            metadata = ToolMetadata(name="Wikipedia Search",description ="Useful for performing searches on Wikipedia knowledgebase"),
            ),
    ]
