...
def wikipage_list(query):
    ...
    element list.
    """
    program = OpenAIPydanticProgram.from_defaults(output_cls = WikiPageList, prompt = prompt_template_str, verbose=True)
    wikipage_requests = program(query=query)
    print('Wikipage_requests: {}'.format(wikipage_requests))
